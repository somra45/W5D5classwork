def it_was_ok
  ...
  Movie.select(:id, :title, :score).where('score BETWEEN ? AND ?', 2, 3) # 'score BETWEEN 2.0 AND 3.0' or score: (2.0..3.0)
end
...
def biggest_cast
  ...
  # actors).
  Movie.joins(:actors).group(:id).order('COUNT(actor_id) DESC').limit(3).select(:id, :title)
